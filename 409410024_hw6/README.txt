...
hw4_test.s:
	...
	打開一個file，把排序後array內的數字轉換成字串(sprintf)並合併成一個字串(strcat)後寫進file裡面
	最後再close file
numsort.s:(跟hw5一樣)
...
關於編譯執行等的部分：
首先打開wsl
在cd到適當位置後輸入 ./bin/arm-none-eabi-gcc -g hw6_test.s numsort.s -o hw6.exe 產生exe執行檔
